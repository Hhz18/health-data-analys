...
public class HeiLongJiangNewsCrawler implements NewsCrawler {
    private final String pageUrl ="http://wsjkw.hlj.gov.cn/wsjkw/c109060/list.shtml";
    ...
    public List<NewsItem> fetchNewsByDate(LocalDate date) {
        ...
        try{
            Document doc = Jsoup.connect(pageUrl)
                    ...
                    .timeout(10000)
                    .get();
            Elements blocks = doc.select("div.listright >ul>li"); //直接选择ul下所有 li（推荐）

            String  targetStr =date.toString();

            for (Element block:blocks){
                String title = block.select("a").text().trim();
                String dateStr = block.select("a >span").text().trim();
            ...
            System.err.println("❌ 黑龙江省爬虫失败：" + e.getMessage());
        }
        return result;
    }
    @Override
