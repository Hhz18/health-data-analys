...
public class LiaoNingNewsCrawler implements NewsCrawler {
    private final String pageUrl ="https://wsjk.ln.gov.cn/wsjk/index/syxwxx/wsjkkx/index.shtml";
    ...
    public List<NewsItem> fetchNewsByDate(LocalDate date) {
        ...
        try{
            Document doc = Jsoup.connect(pageUrl)
                    ...
                    .timeout(10000)
                    .get();
            Elements blocks = doc.select("div.jgsz_right_con >ul>li"); //直接选择ul下所有 li（推荐）

            String  targetStr =date.toString();

            for (Element block:blocks){
                String title = block.select("a").text().trim();
                String dateStr = block.select("span.datetime").text().trim();
            ...
            System.err.println("❌ 辽宁省爬虫失败：" + e.getMessage());
        }
        return result;
    }
    @Override
